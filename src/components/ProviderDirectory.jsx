  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-4 mb-4 shadow-sm">
        <div className="flex items-center justify-between mb-3">
          <div>
            <p className="text-xs text-slate-600">
              ...
              {isSimulationMode && (
                <span className="ml-2 text-amber-600">• Simulation Mode Active</span>
              )}
            </p>
          </div>
          {isSimulationMode && (
        ...
        <div className="grid grid-cols-4 gap-3">
          <input
            ...
            className="col-span-2 px-3 py-2 bg-slate-50 border border-slate-300 rounded-lg text-xs text-slate-900 placeholder-slate-500 focus:border-teal-500 focus:ring-1 focus:ring-teal-500 outline-none"
          />

          <select
            value={confidenceFilter}
            onChange={(e) => { setConfidenceFilter(e.target.value); setCurrentPage(0); }}
            className="px-3 py-2 bg-slate-50 border border-slate-300 rounded-lg text-xs text-slate-900"
          >
        ...
        <div className="flex items-center gap-4 mt-3 text-xs">
          <span className="text-slate-600">Confidence Distribution:</span>
          <span className="flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full bg-emerald-500"></span>
            <span className="text-slate-900 font-medium">{confidenceStats.High.toLocaleString()}</span>
            <span className="text-slate-600">High</span>
          </span>
          <span className="flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full bg-amber-500"></span>
            <span className="text-slate-900 font-medium">{confidenceStats.Mid.toLocaleString()}</span>
            <span className="text-slate-600">Mid</span>
          </span>
          <span className="flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full bg-rose-500"></span>
            <span className="text-slate-900 font-medium">{confidenceStats.Low.toLocaleString()}</span>
      ...
      <div className="flex-1 bg-white border border-slate-200 rounded-2xl overflow-hidden flex flex-col shadow-sm">
        <div className="flex-1 overflow-y-auto">
          <table className="w-full text-xs">
            <thead className="sticky top-0 bg-slate-50 backdrop-blur-lg border-b border-slate-200">
              <tr>
                <th className="text-left px-4 py-3 text-slate-600 font-medium">Provider</th>
                <th className="text-left px-4 py-3 text-slate-600 font-medium">NPI</th>
                <th className="text-left px-4 py-3 text-slate-600 font-medium">Specialty</th>
                <th className="text-left px-4 py-3 text-slate-600 font-medium">Location</th>
                <th className="text-left px-4 py-3 text-slate-600 font-medium">Confidence</th>
                ...
                return (
                  <tr
                    ...
                    className={`border-b border-slate-200 hover:bg-slate-50 transition-colors cursor-pointer ${
                      selectedProvider?.id === provider.id ? 'bg-slate-100' : ''
                    } ${isRemoved ? 'opacity-40' : ''}`}
                    onClick={() => setSelectedProvider(provider)}
                  >
                    <td className={`px-4 py-3 ${isRemoved ? 'text-slate-400 line-through' : 'text-slate-900'}`}>
                      {provider.name}
                    </td>
                    <td className={`px-4 py-3 font-mono ${isRemoved ? 'text-slate-400 line-through' : 'text-slate-600'}`}>
                      {provider.npi}
                    </td>
                    <td className={`px-4 py-3 ${isRemoved ? 'text-slate-400 line-through' : 'text-slate-700'}`}>
                      {provider.specialty}
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <span className={`px-2 py-0.5 rounded ${isRemoved ? 'bg-slate-200 text-slate-400 line-through' : 'bg-slate-100 text-slate-700'}`}>
                          {provider.state}
                        </span>
                        {onShowOnMap && (
                          <button
                    ...
                    <td className="px-4 py-3">
                      <span className={`px-2 py-0.5 rounded-full text-xs ${
                        isRemoved ? 'bg-slate-200 text-slate-400 line-through' :
                        provider.confidence === 'High' ? 'bg-emerald-500/20 text-emerald-600' :
                        provider.confidence === 'Mid' ? 'bg-amber-500/20 text-amber-600' :
                    ...
                    <td className="px-4 py-3">
                      {isRemoved ? (
                        <span className="text-slate-400 text-xs line-through">Removed</span>
                      ) : provider.acceptingPatients ? (
                        <span className="text-emerald-600 text-xs">Accepting</span>
                      ) : (
                    ...
                    <td className="px-4 py-3 text-right">
                      {isRemoved ? (
                        <button
                          ...
                          className="px-3 py-1 bg-teal-600 hover:bg-teal-700 text-white rounded text-xs transition-colors"
                        >
                          Restore
                        </button>
                      ) : (
          ...
          <div className="border-t border-slate-200 bg-slate-50 px-4 py-3 flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="px-3 py-1.5 bg-white hover:bg-slate-100 text-slate-700 text-xs rounded border border-slate-300 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
                Previous
              </button>
              <button
      ...
      {selectedProvider && (() => {
        ...
        return (
          <div className="fixed right-4 top-20 w-80 bg-white backdrop-blur-lg border border-teal-500/50 rounded-xl p-4 shadow-xl z-50">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <h3 className="font-bold text-slate-900 text-sm">Provider Details</h3>
                {isRemoved && (
                  <span className="px-2 py-0.5 bg-rose-500/20 text-rose-600 text-xs rounded-full">
                    Removed
                  </span>
                )}
              </div>
            ...
            <div className="space-y-3 text-xs">
              <div>
                ...
                <p className="text-slate-900 font-medium">{selectedProvider.name}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="text-slate-900">{selectedProvider.specialty}</p>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <p className="text-slate-600 mb-1">State</p>
                  <p className="text-slate-900">{selectedProvider.state}</p>
                </div>
                <div>
                  <p className="text-slate-600 mb-1">Years in Practice</p>
                  <p className="text-slate-900">{selectedProvider.yearsInPractice}</p>
                </div>
              </div>
              <div>
                <p className="text-slate-600 mb-1">Confidence Rating</p>
                <span className={`px-2 py-1 rounded-full text-xs inline-block ${
                  selectedProvider.confidence === 'High' ? 'bg-emerald-500/20 text-emerald-600' :
                  selectedProvider.confidence === 'Mid' ? 'bg-amber-500/20 text-amber-600' :
                  'bg-rose-500/20 text-rose-600'
                }`}>
              ...
              <div>
                <p className="text-slate-600 mb-1">Accepting Patients</p>
                <p className={selectedProvider.acceptingPatients ? 'text-emerald-600' : 'text-slate-500'}>
            ...
            <div className="mt-4 pt-4 border-t border-slate-200">
              {isRemoved ? (
                <button
                  ...
                  className="w-full px-3 py-2 bg-teal-600 hover:bg-teal-700 text-white rounded text-xs transition-colors flex items-center justify-center gap-2"
                >
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                  </svg>
